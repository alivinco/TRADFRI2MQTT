...
/**
 * The {@link ColorConverter} is used for conversion between color formats.
 * Use the static methods {@link ColorConverter#fromCie(int, int, int)} and {@link ColorConverter#fromHSBType(HSBType)} for
 * construction.
 *
...
public class ColorConverter {
     ...
     */
    private ColorConverter(Integer rgbR, Integer rgbG, Integer rgbB, Integer xyX, Integer xyY, Integer brightness,
                         HSBType hsbType) {
        super();
        this.rgbR = rgbR;
        this.rgbG = rgbG;
     ...
     * @return {@link ColorConverter} object with converted color spaces
     */
    public static ColorConverter fromCie(int xyX, int xyY, int xyBrightness) {
        ...
        int rgbR = (int) (hsbType.getRed() * 2.55);
        int rgbG = (int) (hsbType.getGreen() * 2.55);
        int rgbB = (int) (hsbType.getBlue() * 2.55);

        return new ColorConverter(rgbR, rgbG, rgbB, xyX, xyY, brightness, hsbType);
    }
     ...
     * @return {@link ColorConverter} object with converted color spaces
     */
    public static ColorConverter fromHSBType(HSBType hsbType) {

        // hsbType gives 0 to 100, we need 0.0 to 255.0
        double red = hsbType.getRed() * 2.55;
        double green = hsbType.getGreen() * 2.55;
        double blue = hsbType.getBlue() * 2.55;
        ...
        int xyX = normalize(x);
        int xyY = normalize(y);
        int brightness = (int) (hsbType.getBrightness() * 2.54);
        ...
        return new ColorConverter(rgbR, rgbG, rgbB, xyX, xyY, brightness, hsbTypeConcreteValues);
    }

    /**
     * Construct from color temperature in percent.
     * 0 (coldest) to 100 (warmest).
     * Note: The resulting {@link ColorConverter} has only the {@link ColorConverter#xyX X} and {@link ColorConverter#xyY y}
     * values set!
     *
     * @param percent the color temperature in percent
     * @return {@link ColorConverter} object with x and y values
     */
    public static ColorConverter fromColorTemperature(int percent) {
        int x, y;
        if (percent < 50.0) {
        ...
        return new ColorConverter(null, null, null, x, y, null, null);
    }
    ...
    private static HSBType constructHsbTypeFromRgbWithBrightnessPercent(int rgbR, int rgbG, int rgbB,
        ...
        int brightnessPercent = xyBrightnessToPercentType(xyBrightness);
     ...
     * @return int with color temperature (0 = coolest, 100 = warmest)
     */
    public static int calculateColorTemperature(int xyX, int xyY) {
        double x = xyX;
        double y = xyY;
        double value = 0.0;
     ...
     *
     * @param xyBrightness xy brightness level 0 to 254
     * @return int with brightness level (0 = light is off, 1 = lowest, 100 = highest)
     */
    public static int xyBrightnessToPercentType(int xyBrightness) {
        if (xyBrightness > 254) {
            xyBrightness = 254;
        } else if (xyBrightness < 0) {
            xyBrightness = 0;
        }
        return ((int) Math.ceil(xyBrightness / 2.54));
    }
